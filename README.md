...
After editing any `poker/js/*.js` file, regenerate the single-request
bundle with `node tools/build.js` (the deploy CDN rate-limits parallel
script requests, so the page loads one `bundle.js`).
